...
class SpeakerModel:
    ...
    def derivational_search_function(self, X, sentence, index):
        if index == len(sentence):
            ...
            new_item = self.lexicon.lexical_retrieval(sentence[index])
            for i, right_edge in enumerate(X.right_edge()):
                X_ = X.root().copy()
                right_edge_ = X_.right_edge()[i]
                new_ps_ = right_edge_.MergeRight(new_item)
                self.derivational_search_function(new_ps_, sentence, index + 1)
